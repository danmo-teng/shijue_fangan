...
yaw rate同时保留连续四元数yaw差分和T265 Pose角速度投影两路结果；当前日志验证了镜头朝上
时角速度投影到机器人世界上方向与四元数yaw速率一致。轮式里程计/EKF预测使用T265陀螺
高频偏航增量，T265四元数继续负责绝对航向校正，不直接硬编码`angular_velocity.y`。
...
上位机三轮运动学已与当前F407 `Location.c`统一轮序：

```ini
encoder_to_robot_yaw_deg = 0.0
encoder_fusion_weight = 0.25
```
...
`encoder_fusion_weight`范围0～1，只缩放进入EKF的编码器平移；原始轮式里程计和CSV保持100%，
便于诊断。默认0.25，地图启动页可按5%/10%调节。权重降低时，返航T265降权幅度也同步减小，
低于100%时T265位置逐帧校正。轮式旋转结果仍记录到调试日志，但实际轮式里程计/EKF预测的
航向增量使用T265陀螺仪。若后续发现：

- 仅T265轨迹正确；
- T265+编码器轨迹才偏转；

应优先检查三个`encoder_sign`、M1/M2/M3物理轮序和三轮公式；仅在直行/横移实测证明存在安装
角偏差时才调整`encoder_to_robot_yaw_deg`，不要再修改T265轴配置。
...
- 编码器以100 Hz预测平移；
- T265航向持续校正；
- 编码器权重100%时T265位置按20 Hz较低权重纠偏；低于100%时T265逐帧纠偏且降权倍率随
  编码器权重平方减小；
...
轮式进度优先只在融合模式、编码器权重大于0且最近存在新鲜有效轮式增量时启用。仅T265模式
全程逐帧100%采用T265投影位姿，即使进入NAV/RETURN也不降低T265权重、不触发轮式主导逻辑。
...
任务状态机使用四类物资/伤员，并识别本方`safe_red`/`safe_blue`安全区；物资框中心位于本方安全区
框内时直接过滤，不向F407发送该物资抓取坐标。安全区视觉类别只用于目标过滤，不替代地图或下位机
的投送完成判断。
...
默认YOLO置信度阈值：0.30（阈值包含0.30本身）。传统视觉仍作为显式备用，不应删除。

## 10. 连续搬运任务流程

1. 开局只寻找并搬运一次普通物资。
2. 普通mode21、聚集/分离审核和合爪后复审都要求3个不同`audit_id`语义一致；每个新`frame_sequence`生成一个新`audit_id`，同一帧重复发送保持原ID且不累计。合法审核按首件绿色、`MATERIAL_LEGAL+total_count`和单伤员归一化；非法审核只比较总数、危险/未知/伤员混装标志和目的地，不比较左右位置或绿色/核心/mixed的短暂类别抖动。
3. 合爪前审核合法后持续发送`GRAB_CONFIRMED`；若实际含核心，由F407先按编码器前进50 mm再合爪。
4. F407合爪后上报mode23、GRIPPER_CLOSED和CLAW_VISIBLE；上位机一看到mode23就清除旧审核并建立新frame floor，不以首次采样仍为140°作为进入复审的额外门槛。F407有限恢复短暂切到138°/142°时，上位机只发同一audit_id的非STABLE空审核；离开及回到140°都清空命中并重设frame floor，禁止跨视角拼接3帧。
5. 合爪后合法审核经`AUDIT_VALID(bit4)`和mode22确认后，才按实际类别、数量和目的地锁存并发送NAV。
6. 第一件正式绿色完成后，夹内实际1～3件普通/核心物资可直接运输，伤员仍须单独1件；危险、未知、超过3件或伤员混装继续处理。
7. 普通/核心物资送往物资半区中心；伤员送往伤员半区中心。
8. 投送完成后张爪退出，返回固定场地中心，再搜索下一件。

聚集目标例外：APPROACH期间F407保持双爪完全打开，上位机持续发送CLUSTER_TARGET直到mode38；
夹内审核使用大ROI、独立左右爪ROI、检测框底部中心和有效重叠比例。伤员和核心使用60%重叠门槛，
其他类别保持80%；大ROI按x=640无缝平分左右爪，物体底部中心决定侧向。所有最终抓取与非法处理
都以连续3个新帧为准，任意不一致帧重新开始统计。

夹内审核仅在STM新鲜、CLAW_VISIBLE且相机140°时启用。mode21、mode38和新鲜mode35都会重设frame
floor并清除旧审核/选侧缓存。左右绿色数量只在上位机本地参与确定性强制选侧，不进入UART。

首件绿色中心混堆且普通曲线无法直接留下单绿时，最多一次发送DISPERSE bit5轻撞；F407完成后直接
mode3，上位机清空旧目标用新帧重搜。SEARCH按120°一圈、90°一圈累计约720°，仍无候选则发送
RETURN_CENTER回场地中心，完成后清零累计并从120°重新搜索。
...
- 规划文件超过250 ms没有更新时停止代发；当前F407正常Task不再依赖任务帧龄自动停车，任务线程
  在STM状态失联时（启动自主阶段除外）持续生成PAUSE，恢复后重发当前阶段合法命令解除PAUSE。
...
轮式里程计；融合轨迹仍使用橙色。CSV中的`odom_*`是以T265陀螺高频航向增量积分、未经过
T265姿态校正的轮式累计位姿，`t265_*`是T265场地投影结果，`fused_*`是EKF输出；地图和CSV
中的融合-里程计差异是传感器一致性指标，不等同于带真值的绝对误差。
...
5. 若仅融合模式平移方向错误，只调整编码器平面轴/符号，不再改T265轴；轮式航向由T265陀螺
   增量提供。
