...
1. 开局只寻找并搬运一次普通物资。
2. 普通mode21和聚集/分离后的合法GRAB都要求2个不同`frame_sequence`内容一致；无效审核分侧保持1帧。
3. 持续发送`GRAB_CONFIRMED`，等待新鲜`GRIPPER_CLOSED=1`。
4. 第一件正式绿色完成后，夹内实际1～3件普通/核心物资可直接运输，伤员仍须单独1件。
...
夹内审核使用大ROI、独立左右爪ROI、检测框底部中心和有效重叠比例。聚集筛选默认只剩本轮锁定类别
的1件目标连续两帧才允许GRAB；大ROI按x=640无缝平分左右爪，物体底部中心决定侧向。无效审核的
带侧曲线分离每次mode35后只用frame floor之后的第1个新帧决策。
