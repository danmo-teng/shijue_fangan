...
1. 开局只寻找并搬运一次普通物资。
2. 普通mode21、聚集/分离审核和合爪后复审都要求3个不同`frame_sequence`内容一致；同一帧重复发送不累计。
3. 合爪前审核合法后持续发送`GRAB_CONFIRMED`；若实际含核心，由F407先按编码器前进50 mm再合爪。
4. F407合爪后上报mode23、保持140°和CLAW_VISIBLE；上位机清除旧审核并重新取得连续3帧。F407有限恢复短暂切到138°/142°时，上位机只发非STABLE空审核；离开及回到140°都清空命中并重设frame floor，禁止跨视角拼接3帧。
