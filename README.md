...
MJPEG 1280×1024 @ 180 FPS
  ...
  -> BPU YOLOv8s物资与红/蓝安全区识别（置信度>=0.30）
  -> 传统视觉仅作为显式回退
```
...
返航”测试。新接口与当前任务`0x17/0x18`含义分离，不改动现有任务通信。
...
`competition_rescue/`是独立的完整比赛流程程序：保留旧地图/定位调试程序的出发区、红蓝方、
融合/仅T265、编码器权重、T265地图和T265平移比例控制；新增开局临时藏堆、首件普通物资解锁、
普通/核心批次、伤员单件、危险换道、左右爪审核、边界回转以及停滞退让/旋转脱困策略。旧的
`mission_test/`测试程序不改。新程序的F407配合接口见
`docs/f407_competition_flow_handoff.md`，在下位机更新接口前不要用新程序驱动车辆。
