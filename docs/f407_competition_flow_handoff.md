...
> **当前配合版本说明（2026-09-10，以上位机`codex/gamepad-teleop`最新提交为准）**
>
...
上位机只在`initial_stash=True`的夹内审核中放宽类别限制。F407需要同步调整
`task_validate_audit()`：

- `initial_stash=True`时允许稳定的1～3件物资，不按绿色、核心、伤员、危险或未知类别拒绝；
- 仍拒绝总数为0、总数超过3、左右数量之和不等于总数、审核不稳定的帧；
- `initial_stash=False`时继续执行正式投送硬联锁：首件恰好1件绿色，物资批次不超过3件，
  伤员只能单独1件，危险/未知/伤员混装不能进入正式投送。

### 2. 临时藏堆后的返中

开局藏堆释放完成后，上位机只发送`RETURN_CENTER`，直到发送`D=0`并收到F407新鲜`mode=3`。
返中期间不处理视觉目标、不启动危险换道、不进入SEARCH。

F407继续使用当前`mode=17 FACE_FIELD_CENTER`和RETURN_CENTER反向车头逻辑。不要用普通HOLD
提前把返中转换为SEARCH；HOLD只能在已经完成RETURN且F407处于SEARCH时作为搜索心跳。
...
上位机随后在`mode=15`下确认视觉投送，发送`TASK_COMPLETE`。F407完成张爪后进入
