...
> 协议升级优先要求：[审核READY、帧龄与任务/动作归属](f407_context_ready_freshness.md)。
> 新增0x1D/0x1E配对上下文及状态bit6 AUDIT_READY，须同步更新F407与上位机转发程序。
> 1秒投送策略保持不变；以下历史“单独0x18即可执行”描述不适用于升级后的正式比赛。
...
> 当前最高优先级配合要求：[两件上限、转向放置及限距找回](f407_two_item_turn_place_request.md)。
> 已核对下位机c71efd6；新增mode44/45/46，视觉CLEAR的P4/P5改为±200，
> 不再使用历史150 mm横移暂放。下位机需配套修改后才能运行新扫障流程。

> 最新覆盖说明：本次改为不依赖地面标定的视觉扫障，新增mode42/43及扫障审核标志。
> 请优先执行[无测距视觉扫障配合要求](f407_visual_sweep_request.md)。
> 无障碍保持原ENTER流程；本文以下历史距离门禁/80～600 mm强制要求不再适用于新视觉扫障。
