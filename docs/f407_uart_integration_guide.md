...
如果已经应用过上一版连续搬运补丁`53ddafb`，不要重复执行上面的`git am`，改用增量升级：

```bash
git apply /home/sunrise/RDK_X5/shijue_fangan/docs/f407_patches/0002-use-heading-distance-without-live-pose.patch
```
...
10. 注入`TYPE=0x18`导航、对正、完成和返中命令，确认航向与编码器定距均生效。
11. 连续注入多帧不同SEQ的`GRAB_CONFIRMED`，确认只启动一次夹爪动作，但每个合法SEQ都能更新ACK；置`GRIPPER_CLOSED`后，RDK才开始发送导航命令。
12. 定距尚未开始时中断命令应停车等待；定距已经开始后长时间失联应故障停车，不能重跑完整距离。
13. 注入ABORT后任何NAV都不得恢复。
