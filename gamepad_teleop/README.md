...
按 X 后，本程序继续独占真实 UART，并创建伪终端，把 F407 返回的 ODOM/状态字节镜像给 `t265_map`。`t265_map` 写回伪终端的运动命令会被丢弃，所以建图界面不会与手柄争抢运动控制；手柄仍可连续遥控小车。按 Y 或界面按钮会向 T265 进程发送 SIGINT 并关闭 PTY。
