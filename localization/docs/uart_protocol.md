...
命令文件必须恰好15字节，只允许`TYPE=0x11/0x12/0x18`且CRC正确。配置和视觉报告按新文件
转发一次；任务命令由独立串口线程以100 Hz更新SEQ/CRC并发送，不依赖T265是否返回新帧。
命令文件250 ms没有更新时停止任务心跳。STM32状态JSON由合法`TYPE=0x17`生成，并附带实际
转发计数、错误数、最后SEQ和发送年龄。
