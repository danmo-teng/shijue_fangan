...
- 红方入口边界目标：`(0,+1200 mm)`，正对方向`90°`；安全区目标中心`(0,+1320 mm)`。
- 蓝方入口边界目标：`(0,-1200 mm)`，正对方向`270°`；安全区目标中心`(0,-1320 mm)`。
...
发送`TASK_COMPLETE`还必须同时满足：融合位姿有效、地图小车圆仍与本方安全区相交、普通物资
中心连续3帧位于本方安全区识别框内。这样远处看到安全区时不会提前宣告放入完成。

## 安全区视觉完成条件

进入安全区阶段只使用`green_supply`和本方`safe_red/safe_blue`。地图圆形相交是硬前提；在此前提下，普通物资检测框中心还需连续3帧位于安全区检测框内部（保留3%边缘裕量）才发送`TASK_COMPLETE`。目前没有根据物资面积发送停车信号。
