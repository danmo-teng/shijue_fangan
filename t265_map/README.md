...
桌面入口默认是“观测/记录模式”，会监听 `/dev/ttyS1` 的 ODOM 和已知 F407 状态，但不会发送运动命令。这样在 F407 新接口尚未实现前也可以安全使用 T265 建图和手动推车记录。
...
这个接口只写入本上位机仓库的协议定义和发送端，F407 仍由电控侧另行实现；在下位机实现前不要使用 `--enable-motion`：
