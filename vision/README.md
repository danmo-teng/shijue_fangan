...
- 使用320x256图像快速筛选候选，再回到1280x1024原图复核轮廓、面积、形状和坐标；多帧匹配与UART始终使用原生1280x1024坐标；
...
独立识别程序仍会显示全部六类；连续任务状态机读取本方`safe_blue/safe_red`框，用于过滤已经
位于安全区内的物资，避免把该物资坐标发送给F407抓取。安全区识别不用于替代地图/下位机的投送
完成判断。
