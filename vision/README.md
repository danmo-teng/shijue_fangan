...
- 识别、面积阈值、多帧匹配和UART坐标全部使用1280x1024原图；不再运行640x512粗筛或640x480坐标缩放；
...
编辑器和运行程序会显示实际解码及三级检测帧率。配置中的危险120Hz、普通90Hz、安全区30Hz是调度上限；JPU设置为60Hz时，各检测分支不会超过新解码帧率。当前固定在1280x1024原图进行检测；只有切换到“白黑掩膜”时才生成调试掩膜。如果单次检测长期超过16ms：
